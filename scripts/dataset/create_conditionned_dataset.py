...
def process_batch(batch, target, max_timesteps=None, freq=0, 
    ...
    for t in target:
        ...
        for ts in series:
            ...
            if is_all_nan(ts):
                print(f"All NaN detected for dataset {dataset_name}, target {target_name}, domain {t['domain']}. Skipping series.")
                filtered_counter.increment()
                continue

            if is_constant_ignoring_nan(ts):
                #if rng.random() >= keep_constant_prob:
                print(f"Constant series detected for dataset {dataset_name}, target {target_name}, domain {t['domain']}. Skipping series.")
                filtered_counter.increment()
                continue

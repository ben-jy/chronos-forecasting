...
target_timesteps = min(TOTAL_TIMESTEPS.values())

keep_prob = {
    domain: min(1.0, target_timesteps / total) for domain, total in TOTAL_TIMESTEPS.items()
}

print("Keep probabilities per domain:")
for d, p in keep_prob.items():
    print(f"  {d}: {p:.6f}")
...
def process_batch(batch, target, max_timesteps=None, freq=0, 
    ...
    for t in target:
        ...
        for ts in series:
            ...
            if t['domain'] is not None and rng.random() > keep_prob[t['domain']]:
